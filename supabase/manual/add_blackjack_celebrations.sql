-- Apply after restore_game_cosmetics.sql. Safe to repeat on the same catalog.
-- Adds three coin-purchased celebrations without updating or deleting any player data.
-- Targets the currently deployed 12-item catalog, not the separate premium-avatar branch.
begin;
...
begin
  ...
  loop
    execute format(
      'select coalesce(jsonb_agg(row order by row::text), ''[]''::jsonb) from (select to_jsonb(t) as row from public.%I t) s',
      target
    ) into snapshot;
...
-- Refuse to replace a different catalog. Preserve later expansions rather than narrowing them.
do $$
begin
  if exists (select 1 from private.cosmetic_catalog('royal-avatar')) then
    raise exception 'Premium-avatar catalog detected; merge this expansion with that catalog before applying';
  end if;
end;
$$;
...
alter table public.player_cosmetics
  ...
  add constraint player_cosmetics_cosmetic_id_check check (cosmetic_id in (
    ...
    'golden-blackjack', 'royal-blackjack', 'electric-blackjack'
  ));
...
as $$
  ...
  from (values
    ...
    ('golden-blackjack', 'blackjack-celebration', 180::bigint),
    ('royal-blackjack', 'blackjack-celebration', 220::bigint),
    ('electric-blackjack', 'blackjack-celebration', 200::bigint)
...
begin
  ...
  if p_slot is null or p_slot not in ('frame', 'card-back', 'card-animation', 'tank-decal', 'blackjack-celebration') then
...
-- Abort everything if any existing data changed.
do $guard$
declare
  saved record;
  current_data jsonb;
