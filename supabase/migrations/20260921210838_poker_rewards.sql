-- Allow Poker match outcomes through the existing server-only, idempotent reward path.
...
begin
  if nullif(trim(p_match_id), '') is null
     or p_game_id not in ('bomb-party', 'tank-arena', 'blackjack-party', 'poker-party')
